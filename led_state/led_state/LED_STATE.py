...
from .modules.spi_module import SPItoWS
...
def main():
    try:        
        ...
        while(1):
            try:
                with open('/home/refro/ros2_ws/src/led_state/led_msg.txt') as f:
                    lines = f.readlines()
                    # _buffer['state'] = int(next(f).split(':')[1])
                    # _buffer['battery'] = int(next(f).split(':')[1])

                    _buffer['state'] = int(lines[0].split(':')[1])
                    _buffer['battery'] = int(lines[1].split(':')[1])
                    # print(_buffer)
            except IndexError:
